									...
									struct fsm_misc_args *args)
{
	...
	case FSM_IOC_CALIBRATE:
		cfg->store_otp   = args->store_otp;
		cfg->force_calib = args->force_calib;
		fsm_re25_test(cfg->force_calib);
		break;
	case FSM_IOC_F0_TEST:
		cfg->freq_start = args->freq_start;
		cfg->freq_end   = args->freq_end;
		cfg->freq_step  = args->freq_step;
		cfg->freq_count = args->freq_count;
		pr_info("start:%d, step:%d, end:%d", cfg->freq_start,
				cfg->freq_step, cfg->freq_end);
		fsm_f0_test();
		break;
	default:
...
static int fsm_get_test_result(void *arg)
{
	fsm_config_t *cfg = fsm_get_config();
	...
	size = cfg->freq_count * FSM_DEV_MAX * sizeof(uint16_t)
	       + sizeof(struct fsm_cal_result);
	...
	if (ret) {
		pr_err("get result fail:%d", ret);
		fsm_free_mem(result);
		return ret;
	}
	ret = copy_to_user((void *) arg, result, size);
	if (ret) {
...
int fsm_misc_send_apr(void __user *buf)
{
	...
	if (header[4] == FSM_OPCODE_GET_PARAMS) {
		ret = copy_from_user(buffer, (void *) buf, sizeof(header));
		if (ret) {
			pr_err("copy from user fail:%d", ret);
			goto exit;
		}
		ret = fsm_algo_send_afe_cal(buffer);
		if (ret) {
			pr_err("get params fail:%d", ret);
			goto exit;
		}
		ret = copy_to_user((void __user *) buf, buffer, buf_size);
		if (ret) {
			pr_err("copy to user fail:%d", ret);
			goto exit;
		}
	} else if (header[4] == FSM_OPCODE_SET_PARAMS) {
		...
		pr_err("invalid opcode:%d", header[4]);
		ret = -EINVAL;
	}
exit:
	fsm_free_mem(buffer);
							...
							unsigned long arg)
{
	...
	case FSM_IOC_GET_RESULT:
		ret = fsm_get_test_result((void *) arg);
		if (ret) {
			pr_err("get result fail:%d", ret);
		}
		break;
