...
static int aw87xxx_power_down(struct aw87xxx *aw87xxx, char *profile)
{
	...
	if (aw_dev->hwen_status == AW_DEV_HWEN_OFF) {
		AW_DEV_LOGD(aw87xxx->dev, "profile[%s] has already load ", profile);
	} else {
...
char *aw87xxx_show_current_profile(int dev_index)
{
	...
	list_for_each(pos, &g_aw87xxx_list) {
		...
		if (aw87xxx->dev_index == dev_index) {
			AW_DEV_LOGD(aw87xxx->dev, "current profile is [%s]",
	...
	list_for_each(pos, &g_aw87xxx_list) {
		...
		if (aw87xxx->dev_index == dev_index) {
			...
			if (vol_desc->addr == AW_REG_NONE) {
				AW_DEV_LOGD(aw87xxx->dev, "PA unsupport set boost voltage!");
				return -EINVAL;
			}
			if (status == AW_VOLTAGE_HIGH) {
				AW_DEV_LOGD(aw87xxx->dev, "set reg voltage max: 0x%02x = 0x%02x",
				...
				AW_DEV_LOGD(aw87xxx->dev, "set reg voltage min: 0x%02x = 0x%02x",
...
int aw87xxx_get_boost_voltage(int dev_index, int *status)
{
	...
	list_for_each(pos, &g_aw87xxx_list) {
		...
		if (aw87xxx->dev_index == dev_index) {
			...
			vol_desc = &aw87xxx->aw_dev.vol_desc;
			if (vol_desc->addr == AW_REG_NONE) {
				AW_DEV_LOGD(aw87xxx->dev, "PA unsupport get boost voltage!");
				return -EINVAL;
			}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	AW_DEV_LOGD(aw87xxx->dev, "set profile [%s]", profile_name);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	AW_DEV_LOGD(aw87xxx->dev, "current profile:[%s]",
		aw87xxx->current_profile);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	AW_DEV_LOGD(aw87xxx->dev, "get vmax = [0x%x]", vmax_val);
						...
						void *codec)
{
	...
	AW_DEV_LOGD(aw87xxx->dev, "add codec controls[%s,%s,%s]",
						...
						void *codec)
{
	...
	AW_DEV_LOGD(aw87xxx->dev, "add public codec controls[%s,%s,%s]",
...
static int aw87xxx_init_default_prof(struct aw87xxx *aw87xxx)
{
	...
	AW_DEV_LOGD(aw87xxx->dev, "init profile name [%s]",
		aw87xxx->current_profile);
...
static void aw87xxx_fw_load_work(struct work_struct *work)
{
	...
	AW_DEV_LOGD(aw87xxx->dev, "acf parse succeed");
			...
			struct device_attribute *attr, char *buf)
{
	...
	AW_DEV_LOGD(aw87xxx->dev, "current profile:[%s]", aw87xxx->current_profile);
			...
			const char *buf, size_t count)
{
	...
	AW_DEV_LOGD(aw87xxx->dev, "buf_len = %d,reg_num = %d", buf_len, packet->reg_num);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	AW_DEV_LOGD(aw87xxx->dev, "string:[%s],count=%ld", buf, (u_long)count);
	...
	if (wr_status == AWRW_FLAG_WRITE) {
		...
		AW_DEV_LOGD(aw87xxx->dev, "read_cmd:reg_addr[0x%02x], reg_num[%d]",
	...
	struct device_attribute *attr, char *buf)
{
	...
	AW_DEV_LOGD(aw87xxx->dev, "reg_addr 0x%02x, reg_num %d",
		packet->reg_addr, packet->reg_num);

	for (i = 0; i < data_len; i++) {
		len += snprintf(buf + len, PAGE_SIZE - len,
			"0x%02x,", reg_data[i]);
		AW_DEV_LOGD(aw87xxx->dev, "0x%02x", reg_data[i]);
	}
				...
				struct device_node *dev_node)
{
	...
	if (ret < 0) {
		AW_DEV_LOGD(aw87xxx->dev, "dev_index parse failed, user default[%d], ret=%d",
				g_aw87xxx_dev_cnt, ret);
		aw87xxx->dev_index = g_aw87xxx_dev_cnt;
	} else {
		aw87xxx->dev_index = dev_index;
		AW_DEV_LOGD(aw87xxx->dev, "parse dev_index=[%d]",
				aw87xxx->dev_index);
	}
	...
	if (ret < 0) {
		AW_DEV_LOGD(aw87xxx->dev, "no reset gpio provided, hardware reset unavailable");
		...
		AW_DEV_LOGD(aw87xxx->dev, "reset gpio[%d] parse succeed", ret);
	...
	if (ret < 0) {
		AW_DEV_LOGD(aw87xxx->dev, "voltage_min parse failed, user default[0x%02x]",
				AW_BOOST_VOLTAGE_MIN);
		vol_desc->vol_min = AW_BOOST_VOLTAGE_MIN;
	} else {
		vol_desc->vol_min = voltage_min;
		AW_DEV_LOGD(aw87xxx->dev, "parse aw_voltage_min=[0x%02x]",
				vol_desc->vol_min);
	}
...
static struct aw87xxx *aw87xxx_malloc_init(struct i2c_client *client)
{
	...
	AW_DEV_LOGD(&client->dev, "struct aw87xxx devm_kzalloc and init down");
	return aw87xxx;
}
...
static int aw87xxx_i2c_probe(struct i2c_client *client)
{
	...
	AW_DEV_LOGD(aw87xxx->dev, "succeed, dev_index=[%d], g_aw87xxx_dev_cnt= [%d]",
			aw87xxx->dev_index, g_aw87xxx_dev_cnt);
