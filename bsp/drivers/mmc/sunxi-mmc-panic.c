...
#define mmc_mreadl(reg_base, reg) \
	({\
		int val = readl((void __iomem *)reg_base + SDXC_##reg);\
		/* printk("%x\n", val); */\
		val;\
	})
...
({\
	int val = 0;\
	writel((value), (void __iomem *)(reg_base + SDXC_##reg));\
...
static void __iomem *gccmu_base_reg;
static void __iomem *ghost_base_reg;
...
#ifdef NCAT
static void sunxi_mmc_mbusrst_host(void __iomem *host)
{
	void __iomem *ccmu_reg = gccmu_base_reg;
...
static void sunxi_mmc_regs_save(void __iomem *host)
{
...
static void sunxi_mmc_regs_restore(void __iomem *host)
{
	struct sunxi_mmc_mbak_regs *bak_regs = &gmbak_regs;
	void __iomem *ccmu_reg = gccmu_base_reg;
	u32 rval = 0;
...
static int sunxi_mmc_mupdate_clk(void __iomem *host)
{
	u32 rval;
	int i = 0;
...
static void sunxi_mmc_rcover_host(void __iomem *host_reg_base)
{
	sunxi_mmc_regs_save(host_reg_base);
	sunxi_mmc_mbusrst_host(host_reg_base);
	sunxi_mmc_regs_restore(host_reg_base);
	sunxi_mmc_mupdate_clk(host_reg_base);
}

static int sunxi_mmc_mchk_r1_rdy(void __iomem *reg, int to_ns)
{
	int i = 0;
	/* wait busy over */
	for (i = 0; i < to_ns; i++) {
...
static int sunxi_mmc_raw_write(void __iomem *reg, u32 sec_addr,
...
static int sunxi_mmc_raw_half_write(void __iomem *reg, u32 sec_addr,
...
static int sunxi_mmc_raw_wcmd_clr(void __iomem *reg, int *out_cmd_val)
{
	int i = 0;
	u32 cmd_val = 0;
...
static void sunxi_mmc_raw_stop(void __iomem *reg)
{
	u32 arg, cmd_val, ri;
	int i = 0;
	int rval = 0;
...
static int sunxi_mmc_raw_read(void __iomem *reg, u32 sec_addr,
...
static int sunxi_mmc_raw_half_read(void __iomem *reg, u32 sec_addr, u32 sec_cnt, u32 stp_wd, char *outbuf)
{
	u32 cmd_val, ri;
	u32 rval;
	int to = 0;
...
static int sunxi_mmc_panic_read(u32 sec_addr, u32 sec_cnt, char *outbuf)
{
	void __iomem *reg = ghost_base_reg;
	int ret = 0;
	u32 cmd_val = 0;
...
/* use for panic situation,no irq,no lock,no dma */
static int sunxi_mmc_panic_write(u32 sec_addr, u32 sec_cnt, const char *inbuf)
{
	int ret = 0;
	u32 cmd_val = 0;
	void __iomem *reg = ghost_base_reg;
...
static int sunxi_mmc_panic_init(void)
{
...
static void sunxi_mmc_panic_exit(void)
{
