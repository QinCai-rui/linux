...
static int register_dcxo24M_out_as_gate(struct sunxi_rtc_ccu *priv, int clk_index)
{
	...
	if (IS_ERR(priv->clks[clk_index])) {
		sunxi_err(dev, "Couldn't register clk '%s'\n", clkname);
...
static int sunxi_rtc_clk_provider_register(struct sunxi_rtc_ccu *priv)
{
	...
	if (!priv->clks) {
		sunxi_err(dev, "Fail to alloc memory for priv->clks\n");
		return -ENOMEM;
	}

	for (i = 0; i < priv->clks_num; i++) {
		sunxi_info(dev, "Registering clk '%s'\n", clk_info_table[i].clk_name);
	...
	if (!clk_data) {
		sunxi_err(dev, "Fail to alloc memory for clk_data\n");
		return -ENOMEM;
	}
	...
	if (err) {
		sunxi_err(dev, "Fail to add clk provider\n");
		return err;
	}
...
static int sunxi_rtc_ccu_probe(struct platform_device *pdev)
{
	...
	if (err) {
		sunxi_err(dev, "sunxi_rtc_clk_provider_register() failed\n");
		return err;
	}

	sunxi_info(dev, "sunxi rtc-ccu probed\n");
	return 0;
}
...
static int __init sunxi_rtc_ccu_init(void)
{
	...
	if (err)
		sunxi_err(NULL, "Fail to register sunxi_rtc_ccu as platform device\n");
