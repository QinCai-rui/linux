...
#include "axp2602_battery.h"

#define DIVIDE_BY_50(n) (((n) / 50) * 50)

struct axp2602_bat_power {
	char                      *name;
	struct device             *dev;
...
static enum power_supply_property axp2602_bat_props[] = {
	/* real_time */
...
static int axp2602_usb_power_get(struct axp2602_bat_power *bat_power)
{
...
static int axp2602_gpio_vbus_power_get(struct axp2602_bat_power *bat_power)
{
...
static int axp2602_can_charge_status_get(struct axp2602_bat_power *bat_power)
{
	return axp2602_usb_power_get(bat_power) || axp2602_gpio_vbus_power_get(bat_power);
}

static int _axp2602_get_vbat_vol(struct axp2602_bat_power *bat_power)
{
	...
	for (i = 0; i < 3; i++) {
		ret = regmap_bulk_read(regmap, AXP2602_VBAT_H, data, 2);
		if (ret < 0)
			return ret;
	...
	/*incase vtemp[1] exceed AXP2602_VBAT_MAX */
	if ((vtemp[1] > AXP2602_VBAT_MAX) || (vtemp[1] < AXP2602_VBAT_MIN)) {
		bat_vol = 0;
	} else {
...
static int axp2602_get_vbat_vol(struct power_supply *ps,
			     union power_supply_propval *val)
{
	struct axp2602_bat_power *bat_power = power_supply_get_drvdata(ps);

	val->intval = _axp2602_get_vbat_vol(bat_power);

	return 0;
}

/* read temperature */
static int axp2602_get_temp(struct power_supply *ps,
			     union power_supply_propval *val)
{
	struct axp2602_bat_power *bat_power = power_supply_get_drvdata(ps);

	if (axp2602_usb_power_get(bat_power)) {
...
static int axp2602_get_bat_health(struct power_supply *ps,
			     union power_supply_propval *val)
{
	struct axp2602_bat_power *bat_power = power_supply_get_drvdata(ps);
...
static int _axp2602_get_bat_present(struct axp2602_bat_power *bat_power)
{
	int bat_vol;

	bat_vol = _axp2602_get_vbat_vol(bat_power);
	if (bat_vol > 3000)
		return 1;
...
static int axp2602_get_bat_present(struct power_supply *ps,
				  union power_supply_propval *val)
{
	struct axp2602_bat_power *bat_power = power_supply_get_drvdata(ps);
	...
	if (axp2602_usb_power_get(bat_power)) {
		...
		val->intval = _axp2602_get_bat_present(bat_power);
	}

	return 0;
}

static int axp2602_get_soc(struct power_supply *ps,
			    union power_supply_propval *val)
{
	struct axp2602_bat_power *bat_power = power_supply_get_drvdata(ps);
	struct regmap *regmap = bat_power->regmap;
	...
	if (axp2602_usb_power_get(bat_power)) {
		...
		axp2602_get_bat_present(ps, val);
	}
	...
	if (!(axp2602_can_charge_status_get(bat_power))) {
		writel(0, bat_power->writebase);
	} else {
		if (axp2602_usb_power_get(bat_power)) {
			vbus_status = bat_power->qc_psy;
		} else if (axp2602_gpio_vbus_power_get(bat_power)) {
	...
	if (reg_value & 0x80 || reg_value & 0x100) {
		...
		ret = regmap_read(regmap, AXP2602_GAUGE_SOC, &data);
		if (ret < 0)
			return ret;
		data &= 0x7F;
	...
	if (data > AXP2602_SOC_MAX)
		data = AXP2602_SOC_MAX;
	else if (data < AXP2602_SOC_MIN)
		data = AXP2602_SOC_MIN;
...
static int axp2602_get_bat_status(struct power_supply *ps,
				  union power_supply_propval *val)
{
	struct axp2602_bat_power *bat_power = power_supply_get_drvdata(ps);
...
static inline int axp2602_ibat_to_mA(u32 reg, int reg_value)
{
	int result;

	reg = (reg >> (reg_value + 1)) & 0xffff;
	result = reg & 0x7fff;
	if (reg & 0x8000) {
		result = -result;
	}
...
static int axp2602_get_ichg_lim(struct power_supply *ps,
			     union power_supply_propval *val)
{
	struct axp2602_bat_power *bat_power = power_supply_get_drvdata(ps);
	struct regmap *regmap = bat_power->regmap;
	...
	if (axp2602_usb_power_get(bat_power)) {
		...
		regmap_read(regmap, AXP2602_COMM_CFG2, &reg_value);
		reg_value &= 0x03;
		regmap_bulk_read(regmap, AXP2602_IBAT_ADC, tmp, 2);
		res = (tmp[0] << 8) | tmp[1];
		val->intval = axp2602_ibat_to_mA(res, reg_value);
	}

	return 0;
}

static int _axp2602_get_cycle_count(struct regmap *regmap)
{
	uint8_t tmp[2];
	u32 res;

	regmap_bulk_read(regmap, AXP2602_CYCLE_H, tmp, 2);
	res = (tmp[0] << 8) | tmp[1];

	return res;
}

static int axp2602_get_cycle_count(struct power_supply *ps,
			     union power_supply_propval *val)
{
	struct axp2602_bat_power *bat_power = power_supply_get_drvdata(ps);
	struct regmap *regmap = bat_power->regmap;

	val->intval = _axp2602_get_cycle_count(regmap);

	return 0;
}

static int axp2602_get_charger_count(struct power_supply *ps,
		union power_supply_propval *val)
{
	struct axp2602_bat_power *bat_power = power_supply_get_drvdata(ps);
	...
	charge_cycle_count = _axp2602_get_cycle_count(regmap);
...
static int axp2602_get_charger_count_current(struct power_supply *ps,
		union power_supply_propval *val)
{
	struct axp2602_bat_power *bat_power = power_supply_get_drvdata(ps);
	struct bmu_ext_config_info *bmp_ext_config = &bat_power->dts_info;
...
static int axp2602_set_ichg(struct axp2602_bat_power *bat_power, int mA)
{
...
static int axp2602_set_status(struct axp2602_bat_power *bat_power, int status)
{
	union power_supply_propval qc_val;
...
static int _axp2602_reset_mcu(struct regmap *regmap)
{
	int ret = 0;

	ret = regmap_update_bits(regmap, AXP2602_RESET_CFG, AXP2602_MODE_RSTMCU, AXP2602_MODE_RSTMCU);
	if (ret < 0)
		return ret;

	ret = regmap_update_bits(regmap, AXP2602_RESET_CFG, AXP2602_MODE_RSTMCU,
				 0);
	if (ret < 0)
		return ret;
...
static int axp2602_reset_mcu(struct axp2602_bat_power *bat_power)
{
	int ret = 0;
	union power_supply_propval qc_val;
	struct regmap *regmap = bat_power->regmap;
...
/**
 * axp2602_get_param - get battery config from dts
 *
 * is not get battery config parameter from dts,
 * then it use the default config.
 */
static int axp2602_get_param(struct axp2602_bat_power *bat_power, uint8_t *para,
			     unsigned int *len)
{
...
static int axp2602_model_update(struct axp2602_bat_power *bat_power)
{
	struct regmap *regmap = bat_power->regmap;
	int ret = 0;
	unsigned int data;
	...
	ret = regmap_update_bits(regmap, AXP2602_GAUGE_CONFIG,
				 AXP2602_BROMUP_EN, AXP2602_BROMUP_EN);
	if (ret < 0) {
	...
	len = AXP2602_MAX_PARAM;
	param = devm_kzalloc(bat_power->dev, AXP2602_MAX_PARAM, GFP_KERNEL);
	if (!param) {
		PMIC_ERR("can not find memory for param\n");
		goto UPDATE_ERR;
	}
	ret = axp2602_get_param(bat_power, param, &len);
	if (ret < 0) {
	...
	for (i = 0; i < len; i++) {
		ret = regmap_write(regmap, AXP2602_GAUGE_BROM, param[i]);
		if (ret < 0) {
			PMIC_ERR("%s %d:reg update, i2c communication err!\n", __func__, __LINE__);
			goto err_param;
		}
	...
	for (i = 0; i < len; i++) {
		ret = regmap_read(regmap, AXP2602_GAUGE_BROM, &data);
		if (ret < 0) {
			PMIC_ERR("%s %d:reg update, i2c communication err!\n", __func__, __LINE__);
			goto err_param;
		}
	...
	ret = regmap_update_bits(regmap, AXP2602_GAUGE_CONFIG, AXP2602_BROMUP_EN,
				 0);
	if (ret < 0)
		goto UPDATE_ERR;

	ret = regmap_update_bits(regmap, AXP2602_GAUGE_CONFIG,
				 AXP2602_CFG_UPDATE_MARK,
				 AXP2602_CFG_UPDATE_MARK);
	if (ret < 0)
		goto UPDATE_ERR;

	/* reset_mcu */
	ret = axp2602_reset_mcu(bat_power);
	if (ret < 0)
		goto UPDATE_ERR;

	mdelay(1000);
	axp2602_set_ichg(bat_power, bat_power->dts_info.pmu_runtime_chgcur);
...
UPDATE_ERR:
	regmap_update_bits(regmap, AXP2602_GAUGE_CONFIG, AXP2602_BROMUP_EN, 0);
	axp2602_reset_mcu(bat_power);

	return ret;
}

static int axp2602_blance_vol(struct axp2602_bat_power *bat_power)
{
	...
	ret = regmap_read(bat_power->regmap, AXP2602_GAUGE_SOC, &vol_in_reg);
	if (ret < 0)
		return ret;
	vol_in_reg &= 0x7F;

	if (vol_in_reg > AXP2602_SOC_MAX)
		vol_in_reg = AXP2602_SOC_MAX;
	else if (vol_in_reg < AXP2602_SOC_MIN)
		vol_in_reg = AXP2602_SOC_MIN;
...
static bool axp2602_model_update_check(struct regmap *regmap)
{
	int ret = 0;
	unsigned int data;

	ret = regmap_read(regmap, AXP2602_GAUGE_CONFIG, &data);
	if (ret < 0)
		goto CHECK_ERR;

	if ((data & AXP2602_CFG_UPDATE_MARK) == 0)
		goto CHECK_ERR;
...
static int axp2602_bat_get_property(struct power_supply *psy,
				enum power_supply_property psp,
				union power_supply_propval *val)
{
	int ret = 0;

	struct axp2602_bat_power *bat_power = power_supply_get_drvdata(psy);
	...
	case POWER_SUPPLY_PROP_STATUS:
		ret = axp2602_get_bat_status(psy, val);
		if (ret < 0)
			return ret;
		break;
	case POWER_SUPPLY_PROP_PRESENT:
		ret = axp2602_get_bat_present(psy, val);
		if (ret < 0)
			return ret;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		ret = axp2602_get_vbat_vol(psy, val);
		if (ret < 0)
			return ret;
		break;
	case POWER_SUPPLY_PROP_CAPACITY:
		ret = axp2602_get_soc(psy, val); // unit %;
		if (ret < 0)
			return ret;
		break;
	case POWER_SUPPLY_PROP_TEMP:
		ret = axp2602_get_temp(psy, val);
		if (ret < 0)
			return ret;
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		ret = axp2602_get_ichg_lim(psy, val);
		break;
	...
	case POWER_SUPPLY_PROP_HEALTH:
		ret = axp2602_get_bat_health(psy, val);
		if (ret < 0)
			return ret;
		break;
	...
	case POWER_SUPPLY_PROP_MANUFACTURER:
		val->strval = AXP2602_MANUFACTURER;
		break;
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		ret = axp2602_get_charger_count_current(psy, val);
		break;
	...
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		ret = axp2602_get_cycle_count(psy, val);
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		ret = axp2602_get_charger_count(psy, val);
		if (ret < 0)
			return ret;
		break;
...
static int axp2602_bat_set_property(struct power_supply *psy,
				enum power_supply_property psp,
				const union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		ret = axp2602_set_ichg(bat_power, val->intval);
		break;
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		...
		ret = axp2602_set_ichg(bat_power, lim_cur);
		break;
	case POWER_SUPPLY_PROP_STATUS:
		ret = axp2602_set_status(bat_power, val->intval);
		break;
	default:
		ret = -EINVAL;
	}
...
static int axp2602_bat_property_is_writeable(struct power_supply *psy,
			     enum power_supply_property psp)
{
	int ret = 0;
	switch (psp) {
...
static const struct power_supply_desc axp2602_bat_desc = {
	.name = "axp2602-battery",
	.type = POWER_SUPPLY_TYPE_BATTERY,
	.get_property = axp2602_bat_get_property,
	.set_property = axp2602_bat_set_property,
	.properties = axp2602_bat_props,
	.num_properties = ARRAY_SIZE(axp2602_bat_props),
	.property_is_writeable = axp2602_bat_property_is_writeable,
};

static int axp2602_init_chip(struct axp2602_bat_power *bat_power)
{
	int ret = 0;
	int val;
	u32 reg;
	...
	ret = regmap_update_bits(bat_power->regmap, AXP2602_RESET_CFG, AXP2602_MODE_SLEEP, 0);
	if (ret < 0) {
		PMIC_DEV_ERR(bat_power->dev, "axp2602 reg update, i2c communication err!\n");
		return ret;
	}
	...
	if (axp2602_usb_power_get(bat_power)) {
		...
		qc_val.intval = _axp2602_get_bat_present(bat_power);
	}
	...
	if (!axp2602_model_update_check(bat_power->regmap)) {
		ret = axp2602_model_update(bat_power);
		if (ret < 0) {
			PMIC_DEV_ERR(bat_power->dev, "axp2602 model update fail!\n");
			return ret;
		}
	}
	PMIC_DEV_DEBUG(bat_power->dev, "axp2602 model update ok\n");
	...
	/* battery ratio check */
	regmap_read(bat_power->regmap, AXP2602_GAUGE_SOC, &val);
	val &= 0x7F;
	reg = readl(bat_power->writebase);
	PMIC_INFO("radio:%d", val);
	if (val > 60) {
		ret = regmap_bulk_read(bat_power->regmap, AXP2602_VBAT_H, data, 2);
...
static irqreturn_t axp2602_irq_handler_bat_stat_change(int irq, void *data)
{
	struct irq_desc *id = irq_to_desc(irq);
	struct axp2602_bat_power *bat_power = data;
	...
	case AXP2602_IRQ_NEWSOC:
		regmap_update_bits(bat_power->regmap, AXP2602_IRQ, 0x02, 0);
		regmap_update_bits(bat_power->regmap, AXP2602_IRQ, 0x02, 0x02);
...
enum axp2602_bat_virq_index {
	AXP2602_VIRQ_BAT_NEW_SOC,
	AXP2602_VIRQ_MAX_VIRQ,
};

static struct axp_interrupts axp_bat_irq[] = {
	[AXP2602_VIRQ_BAT_NEW_SOC] = { "gauge_new_soc",
					  axp2602_irq_handler_bat_stat_change },
};

static int axp2602_bat_dt_parse(struct device_node *node,
...
static void axp2602_bat_parse_device_tree(struct axp2602_bat_power *bat_power)
{
	int ret;
	struct bmu_ext_config_info *bmp_ext_config;
	...
	ret = axp2602_bat_dt_parse(bat_power->dev->of_node, bmp_ext_config);
	if (ret) {
...
static void axp2602_bat_power_monitor(struct work_struct *work)
{
	struct axp2602_bat_power *bat_power =
...
static void axp2602_bat_power_full_curve_monitor(struct work_struct *work)
{
	struct axp2602_bat_power *bat_power =
	...
	if (reg & 0x100) {
		blance_vol = axp2602_blance_vol(bat_power);
		PMIC_DEBUG("blance_vol = %d\n", blance_vol);

		regmap_read(bat_power->regmap, AXP2602_GAUGE_SOC, &reg_value);
		rest_vol = reg & 0x7F;
		reg_value &= 0x7F;
		if (reg_value <= 90) {
		...
		if (blance_vol <= 0) {
			...
			PMIC_DEBUG("%s:rest_vol:%d, AXP2602_GAUGE_SOC:%d\n", __func__, rest_vol, reg_value);
			reg &= ~(0x7F);
			reg |= rest_vol;
			writel(reg, bat_power->writebase);
			schedule_delayed_work(&bat_power->bat_power_full_curve, msecs_to_jiffies(60 * 1000));
...
static void axp2602_bat_power_curve_monitor(struct work_struct *work)
{
	struct axp2602_bat_power *bat_power =
...
static int axp2602_battery_probe(struct platform_device *pdev)
{
	u32 reg;
	int ret = 0;
	...
	PMIC_ERR("axp2602-battery device probe\n");
	if (!of_device_is_available(node)) {
		PMIC_ERR("axp2602-battery device is not configed\n");
		return -ENODEV;
	}

	bat_power = devm_kzalloc(&pdev->dev, sizeof(*bat_power), GFP_KERNEL);
	if (bat_power == NULL) {
		PMIC_ERR("axp2602_bat_power alloc failed\n");
		return -ENOMEM;
	}

	bat_power->name = "axp2602_battery";
	bat_power->dev = &pdev->dev;
	bat_power->regmap = axp_dev->regmap;

	/* for device tree parse */
	axp2602_bat_parse_device_tree(bat_power);
	...
	if (of_find_property(bat_power->dev->of_node, "det_qc_supply", NULL)) {
		...
		PMIC_INFO("axp2602:no quick charge\n");
		bat_power->qc_psy = NULL;
	...
	bat_power->writebase = ioremap(AXP2602_FLAGE_REG, 4);
	...
	ret = axp2602_init_chip(bat_power);
	if (ret < 0) {
		PMIC_DEV_ERR(bat_power->dev, "axp2602 init chip fail!\n");
		ret = -ENODEV;
		goto err;
	}
	...
	bat_power->bat_supply = devm_power_supply_register(bat_power->dev,
			&axp2602_bat_desc, &psy_cfg);

	if (IS_ERR(bat_power->bat_supply)) {
		PMIC_ERR("axp2602 failed to register bat power\n");
	...
	if (axp_dev->irq) {
		...
		PMIC_INFO("axp2602:no irq\n");
	}

	platform_set_drvdata(pdev, bat_power);

	INIT_DELAYED_WORK(&bat_power->bat_supply_mon, axp2602_bat_power_monitor);
	INIT_DELAYED_WORK(&bat_power->bat_power_full_curve, axp2602_bat_power_full_curve_monitor);
	...
	if (reg & 0x80) {
		reg_value = axp2602_blance_vol(bat_power);
		PMIC_DEBUG("bat curve smoothing: hold wake lock,blance_vol = %d\n", reg_value);
		__pm_stay_awake(bat_power->ws);
		INIT_DELAYED_WORK(&bat_power->bat_power_curve, axp2602_bat_power_curve_monitor);
		schedule_delayed_work(&bat_power->bat_power_curve, msecs_to_jiffies(30 * 1000));
	}
...
static int axp2602_battery_remove(struct platform_device *pdev)
{
	struct axp2602_bat_power *bat_power = platform_get_drvdata(pdev);

	PMIC_DEV_DEBUG(&pdev->dev, "==============AXP2602 unegister==============\n");
	if (bat_power->bat_supply) {
		power_supply_unregister(bat_power->bat_supply);
		sunxi_power_unregister_cooler(bat_power->bat_supply);
	}
	PMIC_DEV_DEBUG(&pdev->dev, "axp2602 teardown battery dev\n");

	return 0;
}

static inline void axp2602_bat_irq_set(unsigned int irq, bool enable)
{
	if (enable)
		enable_irq(irq);
	else
...
static void axp2602_bat_virq_dts_set(struct axp2602_bat_power *bat_power, bool enable)
{
	struct bmu_ext_config_info *dts_info = &bat_power->dts_info;

	if (!dts_info->wakeup_new_soc)
		axp2602_bat_irq_set(
			axp_bat_irq[AXP2602_VIRQ_BAT_NEW_SOC].irq,
			enable);

}

static void axp2602_bat_shutdown(struct platform_device *pdev)
{
	struct axp2602_bat_power *bat_power = platform_get_drvdata(pdev);
	u32 reg_value;
...
static int axp2602_bat_suspend(struct platform_device *pdev, pm_message_t state)
{
	struct axp2602_bat_power *bat_power = platform_get_drvdata(pdev);
	u32 reg_value;
	...
	axp2602_bat_virq_dts_set(bat_power, false);
	return 0;
}

static int axp2602_bat_resume(struct platform_device *pdev)
{
	struct axp2602_bat_power *bat_power = platform_get_drvdata(pdev);
	u32 reg_value;
	...
	axp2602_bat_virq_dts_set(bat_power, true);
	return 0;
}

static const struct of_device_id axp2602_bat_power_match[] = {
	{
		.compatible = "x-powers-ext,axp2602-bat-power-supply",
		.data = (void *)AXP2602_ID,
	}, {/* sentinel */}
};
MODULE_DEVICE_TABLE(of, axp2602_bat_power_match);

static struct platform_driver axp2602_bat_power_driver = {
	.driver = {
		.name = "axp2602-bat-power-supply",
		.of_match_table = axp2602_bat_power_match,
	},
	.probe = axp2602_battery_probe,
	.remove = axp2602_battery_remove,
	.shutdown = axp2602_bat_shutdown,
	.suspend = axp2602_bat_suspend,
	.resume = axp2602_bat_resume,
};

module_platform_driver(axp2602_bat_power_driver);

MODULE_AUTHOR("wangxiaoliang <wangxiaoliang@x-powers.com>");
MODULE_DESCRIPTION("axp2602 battery driver");
MODULE_LICENSE("GPL");
