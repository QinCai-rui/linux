...
static bool g2d_bld_out_size_check(g2d_image_enh *src,
				 ...
				 struct g2d_mixer_task *p_task)
{
	...
	if (p_para->op_flag > OP_FILLRECT && p_para->src_image_h.bbuff == 1) {
	...
	if (p_para->op_flag & OP_BLEND && p_para->ptn_image_h.bbuff == 1) {
	...
	if (p_para->op_flag & OP_MASK && p_para->ptn_image_h.bbuff == 1) {
		ret = g2d_set_image_addr(&p_frame->ptn_item,
					 &p_para->ptn_image_h);
		if (ret)
			goto OUT;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 15, 0)
{
	...
	if (IS_ERR(pfile)) {
		G2D_WARN("open /tmp/g2d_rcq_header err\n");
		goto OUT;
	}
...
static __s32 g2d_mixer_task_destory(struct g2d_mixer_task *p_task)
{
	...
	ida_free(&g2d_task_ida, p_task->task_id);
	kfree(p_task->frame);
	kfree(p_task);
			 ...
			 unsigned int frame_len)
{
	...
	if (!task) {
		G2D_WARN("kmalloc g2d_mixer_task fail\n");
		goto OUT;
	}
	...
	task->task_id = ida_alloc_range(&g2d_task_ida, 1, 0, GFP_KERNEL);
	if (task->task_id < 0) {
		goto FREE;
	}
	...
	for (i = 0; i < frame_len; ++i) {
		if (g2d_mixer_frame_setup(&task->frame[i], i)) {
			G2D_WARN("frame %d g2d_mixer_frame_setup fail\n", i);
			goto IDA_REMOVE;
		}
	...
	return task->task_id;
IDA_REMOVE:
	ida_free(&g2d_task_ida, task->task_id);
FREE:
	kfree(task->frame);
